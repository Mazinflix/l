        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.wearableOrder.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.wearable-orders.update", [$wearableOrder->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('customer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="customer_id" id="customer_id">
                                @foreach($customers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $wearableOrder->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('wearable') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="wearable_id" id="wearable_id" required>
                                @foreach($wearables as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('wearable_id') ? old('wearable_id') : $wearableOrder->wearable->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('option') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="option_id" id="option_id" required>
                                @foreach($options as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('option_id') ? old('option_id') : $wearableOrder->option->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantity" id="quantity" value="{{ old('quantity', $wearableOrder->quantity) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $wearableOrder->price) }}" step="0.01" required>
                            @if($errors->has('price'))
