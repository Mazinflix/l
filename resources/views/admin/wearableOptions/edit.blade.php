        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.wearableOption.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.wearable-options.update", [$wearableOption->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('wearable') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="wearable_id" id="wearable_id" required>
                                @foreach($wearables as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('wearable_id') ? old('wearable_id') : $wearableOption->wearable->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('option') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="option" id="option" value="{{ old('option', $wearableOption->option) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="quantity" id="quantity" value="{{ old('quantity', $wearableOption->quantity) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $wearableOption->price) }}" step="0.01" required>
                            @if($errors->has('price'))
                        ...
                        <div class="form-group {{ $errors->has('notes') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="notes" id="notes">{{ old('notes', $wearableOption->notes) }}</textarea>
                            @if($errors->has('notes'))
