        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.walletOrder.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.wallet-orders.update", [$walletOrder->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('customer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="customer_id" id="customer_id">
                                @foreach($customers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $walletOrder->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('wallet') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="wallet_id" id="wallet_id" required>
                                @foreach($wallets as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('wallet_id') ? old('wallet_id') : $walletOrder->wallet->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantity" id="quantity" value="{{ old('quantity', $walletOrder->quantity) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $walletOrder->price) }}" step="0.01">
                            @if($errors->has('price'))
