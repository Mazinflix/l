        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.perfumeOrder.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.perfume-orders.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('perfume') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="perfume_id" id="perfume_id" required>
                                @foreach($perfumes as $id => $entry)
                                    <option value="{{ $id }}" {{ old('perfume_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
