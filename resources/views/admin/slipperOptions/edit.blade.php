        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.slipperOption.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.slipper-options.update", [$slipperOption->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('slipper') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="slipper_id" id="slipper_id" required>
                                @foreach($slippers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('slipper_id') ? old('slipper_id') : $slipperOption->slipper->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('option') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="option" id="option" value="{{ old('option', $slipperOption->option) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="quantity" id="quantity" value="{{ old('quantity', $slipperOption->quantity) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $slipperOption->price) }}" step="0.01" required>
                            @if($errors->has('price'))
