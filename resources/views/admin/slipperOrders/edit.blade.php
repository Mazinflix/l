        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.slipperOrder.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.slipper-orders.update", [$slipperOrder->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('customer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="customer_id" id="customer_id" required>
                                @foreach($customers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $slipperOrder->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('slipper') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="slipper_id" id="slipper_id" required>
                                @foreach($slippers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('slipper_id') ? old('slipper_id') : $slipperOrder->slipper->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('option') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="option_id" id="option_id" required>
                                @foreach($options as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('option_id') ? old('option_id') : $slipperOrder->option->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="quantity" id="quantity" value="{{ old('quantity', $slipperOrder->quantity) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $slipperOrder->price) }}" step="0.01">
                            @if($errors->has('price'))
